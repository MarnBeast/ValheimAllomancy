...
public class Fireplace : MonoBehaviour, Hoverable, Interactable
{
	...
	private void CheckUnderTerrain()
	{
		...
		else if (Physics.Raycast(base.transform.position + Vector3.up * m_coverCheckOffset, Vector3.up, ref val, 0.5f, m_solidRayMask))
		{
			m_blocked = true;
		}
	...
	private void CheckWet()
	{
		...
		Cover.GetCoverForPoint(base.transform.position + Vector3.up * m_coverCheckOffset, ref num, ref flag);
		m_wet = false;
	...
	public bool Interact(Humanoid user, bool hold)
	{
		...
		if (inventory != null)
		{
			if (inventory.HaveItem(m_fuelItem.m_itemData.m_shared.m_name))
			{
				...
				{
					user.Message(MessageHud.MessageType.Center, Localization.get_instance().Localize("$msg_cantaddmore", new string[1]
					{
						m_fuelItem.m_itemData.m_shared.m_name
					}));
					return false;
				}
				user.Message(MessageHud.MessageType.Center, Localization.get_instance().Localize("$msg_fireadding", new string[1]
				{
					m_fuelItem.m_itemData.m_shared.m_name
				}));
	...
	public bool UseItem(Humanoid user, ItemDrop.ItemData item)
	{
		...
		if (m_fireworkItem != null && item.m_shared.m_name == m_fireworkItem.m_itemData.m_shared.m_name)
		{
			...
			user.Message(MessageHud.MessageType.Center, Localization.get_instance().Localize("$msg_throwinfire", new string[1]
			{
				item.m_shared.m_name
			}));
