...
public class Ragdoll : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (vec != Vector3.zero)
		{
			...
			{
				bodies[i].set_velocity(vec * UnityEngine.Random.value);
			}
		}
	}
	...
	private void UpdateFloating(float dt)
	{
		...
		foreach (Rigidbody val in bodies)
		{
			Vector3 worldCenterOfMass = val.get_worldCenterOfMass();
			...
			if (worldCenterOfMass.y < waterLevel)
			{
				...
				val.AddForce(a * dt, (ForceMode)2);
				val.set_velocity(val.get_velocity() - val.get_velocity() * 0.05f * d);
			}
		}
	}
}
