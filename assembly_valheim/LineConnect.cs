...
public class LineConnect : MonoBehaviour
{
	...
	private void SetEndpoint(Vector3 pos)
	{
		...
		if (m_dynamicSlack)
		{
			...
			{
				float num = (float)i / (float)(m_lineRenderer.positionCount - 1);
				float num2 = Mathf.Abs(0.5f - num) * 2f;
				num2 *= num2;
				num2 = 1f - num2;
				Vector3 position2 = Vector3.Lerp(position, b, num);
				position2 += a * d * m_slack * num2;
		...
		{
			float num3 = Vector3.Distance(base.transform.position, pos);
			float f = Utils.LerpStep(m_minDistance, m_maxDistance, num3);
