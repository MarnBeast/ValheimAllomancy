...
public class ZSocket2 : IDisposable, ISocket
{
	...
	public bool StartHost(int port)
	{
		...
		{
			ZLog.LogWarning((object)"Failed to bind socket");
			return false;
		}
		return true;
	}
	...
	private bool BindSocket(int startPort, int endPort)
	{
		for (int i = startPort; i <= endPort; i++)
		{
			try
			{
				...
				m_listner.Start();
				m_listenPort = i;
				ZLog.Log((object)("Bound socket port " + i));
				return true;
			}
			catch
			{
				ZLog.Log((object)("Failed to bind port:" + i));
				m_listner = null;
			}
		}
		return false;
	...
	private void PkgSizeReceived(IAsyncResult res)
	{
		...
		{
			ZLog.LogWarning((object)"PkgSizeReceived socket closed");
			Close();
			return;
		}
		int num;
		...
		{
			ZLog.LogWarning((object)("PkgSizeReceived exception " + ex.ToString()));
			Close();
			return;
		}
		if (num == 0)
		{
			ZLog.LogWarning((object)"PkgSizeReceived Got 0 bytes data,closing socket");
			Close();
			return;
		}
		m_gotData = true;
		...
		{
			ZLog.LogError((object)("PkgSizeReceived Invalid pkg size " + num2));
			return;
		}
	...
	private void PkgReceived(IAsyncResult res)
	{
		...
		{
			ZLog.Log((object)("PkgReceived error " + ex.ToString()));
			Close();
			return;
		}
		if (num == 0)
		{
			ZLog.LogWarning((object)"PkgReceived: Got 0 bytes data,closing socket");
			Close();
			return;
		}
		m_gotData = true;
	...
	public void Send(ZPackage pkg)
	{
		...
		if (!m_isSending)
		{
			...
			{
				ZLog.LogError((object)("Too big data package: " + array2.Length));
			}
			try
			{
			...
			{
				ZLog.Log((object)("Handled exception in ZSocket:Send:" + arg));
				Close();
			}
		}
		else
	...
	private void PkgSent(IAsyncResult res)
	{
		...
		{
			ZLog.Log((object)("PkgSent error " + ex.ToString()));
			Close();
			return;
		}
		m_sendMutex.WaitOne();
		if (m_sendQueue.Count > 0 && IsConnected())
		{
			...
			{
				ZLog.Log((object)("Handled exception in pkgsent:" + arg));
				m_isSending = false;
				Close();
			}
		}
