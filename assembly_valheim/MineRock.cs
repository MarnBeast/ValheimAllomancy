...
public class MineRock : MonoBehaviour, IDestructible, Hoverable
{
	...
	private void Start()
	{
		...
		if ((bool)m_baseModel)
		{
			...
			{
				m_areaMeshes[i] = ((Component)(object)m_hitAreas[i]).GetComponents<MeshRenderer>();
			}
		}
	...
	private void UpdateVisability()
	{
		...
		for (int i = 0; i < m_hitAreas.Length; i++)
		{
			...
			if ((bool)(UnityEngine.Object)(object)val)
			{
				...
				bool flag2 = m_nview.GetZDO().GetFloat(name, m_health) > 0f;
				((Component)(object)val).gameObject.SetActive(flag2);
				if (!flag2)
				{
					flag = true;
				}
	...
	{
		if ((UnityEngine.Object)(object)hit.m_hitCollider == null)
		{
	...
	private void RPC_Hit(long sender, HitData hit, int hitAreaIndex)
	{
		...
		if ((UnityEngine.Object)(object)hitArea == null)
		{
			ZLog.Log((object)("Missing hit area " + hitAreaIndex));
			return;
		}
		string name = "Health" + hitAreaIndex;
		float @float = m_nview.GetZDO().GetFloat(name, m_health);
		if (@float <= 0f)
		{
		...
		m_destroyedEffect.Create(hitArea.get_bounds().center, Quaternion.identity);
	...
	private bool AllDestroyed()
	{
		...
		{
			string name = "Health" + i;
			if (m_nview.GetZDO().GetFloat(name, m_health) > 0f)
			{
				return false;
			}
		}
	...
	private void RPC_Hide(long sender, int index)
	{
		...
		if ((bool)(UnityEngine.Object)(object)hitArea)
		{
			((Component)(object)hitArea).gameObject.SetActive(value: false);
		}
	...
	private int GetAreaIndex(Collider area)
	{
		for (int i = 0; i < m_hitAreas.Length; i++)
		{
			if ((UnityEngine.Object)(object)m_hitAreas[i] == (UnityEngine.Object)(object)area)
			{
				return i;
			}
		}
