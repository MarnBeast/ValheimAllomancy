...
public class StealthSystem : MonoBehaviour
{
	...
	public float GetLightLevel(Vector3 point)
	{
		...
		foreach (Light light in allLights)
		{
			...
			{
				float num2 = 1f;
				if (light.shadows != 0 && (Physics.Raycast(point - light.transform.forward * 1000f, light.transform.forward, 1000f, (int)m_shadowTestMask) || Physics.Raycast(point, -light.transform.forward, 1000f, (int)m_shadowTestMask)))
				{
			...
			if (light.shadows != 0)
			{
				...
				if (Physics.Raycast(light.transform.position, vector.normalized, vector.magnitude, (int)m_shadowTestMask) || Physics.Raycast(point, -vector.normalized, vector.magnitude, (int)m_shadowTestMask))
				{
