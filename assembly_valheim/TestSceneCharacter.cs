...
public class TestSceneCharacter : MonoBehaviour
{
	...
	private void HandleInput(float dt)
	{
		...
		Vector3 vector = zero2 - m_body.get_velocity();
		vector.y = 0f;
		m_body.AddForce(vector, (ForceMode)2);
		base.transform.rotation = m_lookYaw;
		Quaternion rotation = m_lookYaw * Quaternion.Euler(m_lookPitch, 0f, 0f);
		mainCamera.transform.position = base.transform.position - rotation * Vector3.forward * m_cameraDistance;
