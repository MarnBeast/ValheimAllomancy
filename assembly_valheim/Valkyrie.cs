...
public class Valkyrie : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		m_descentStart = m_targetPoint + vector * m_startDescentDistance + a * 200f;
		m_descentStart.y = m_descentAltitude;
		Vector3 a2 = m_targetPoint - m_descentStart;
		a2.y = 0f;
		a2.Normalize();
		m_flyAwayPoint = m_targetPoint + a2 * m_startDescentDistance;
		m_flyAwayPoint.y = m_startAltitude;
		ShowText();
		SyncPlayer(doNetworkSync: true);
		ZLog.Log((object)string.Concat("World pos ", base.transform.position, "   ", ZNet.instance.GetReferencePosition()));
	}
	...
	private void UpdateValkyrie(float dt)
	{
		...
		Quaternion rhs = Quaternion.LookRotation(normalized2);
		...
		rhs = Quaternion.Euler(0f, 0f, num * 45f) * rhs;
		float num2 = (m_droppedPlayer ? (m_turnRate * 4f) : m_turnRate);
		base.transform.rotation = Quaternion.RotateTowards(base.transform.rotation, rhs, num2 * dt);
		Vector3 a = base.transform.forward * m_speed;
		Vector3 vector3 = base.transform.position + a * dt;
		if (ZoneSystem.instance.GetGroundHeight(vector3, out var height2))
		{
			vector3.y = Mathf.Max(vector3.y, height2 + m_dropHeight);
		}
	...
	private void SyncPlayer(bool doNetworkSync)
	{
		...
		localPlayer.GetComponent<Rigidbody>().set_position(localPlayer.transform.position);
		if (doNetworkSync)
		{
