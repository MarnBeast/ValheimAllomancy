...
public class ZConnector : IDisposable
{
	...
	public ZConnector(string host, int port)
	{
		m_host = host;
		m_port = port;
		ZLog.Log((object)("Zconnect " + host + " " + port));
	...
	private void Close()
	{
		if (m_socket != null)
		{
			...
			{
				ZLog.Log((object)("Some excepetion when shuting down ZConnector socket, ignoring:" + arg));
			}
			m_socket.Close();
			m_socket = null;
		}
	...
	public bool UpdateStatus(float dt, bool logErrors = false)
	{
		if (m_abort)
		{
			ZLog.Log((object)"ZConnector - Abort");
			return true;
		}
		if (m_dnsError)
		{
			ZLog.Log((object)"ZConnector - dns error");
			return true;
		}
		if (m_result != null && m_result.IsCompleted)
		{
			ZLog.Log((object)"ZConnector - result completed");
			return true;
		}
		m_timer += dt;
		if (m_timer > m_timeout)
		{
			ZLog.Log((object)"ZConnector - timeout");
			Close();
			return true;
		}
		return false;
	...
	private void OnHostLookupDone(IAsyncResult res)
	{
		...
		if (m_abort)
		{
			ZLog.Log((object)"Host lookup abort");
			return;
		}
		if (iPHostEntry.AddressList.Length == 0)
		{
			m_dnsError = true;
			ZLog.Log((object)"Host lookup adress list empty");
			return;
		}
		ZLog.Log((object)("Host lookup done , addresses: " + iPHostEntry.AddressList.Length));
		IPAddress[] addressList = iPHostEntry.AddressList;
		foreach (IPAddress arg in addressList)
		{
			ZLog.Log((object)(" " + arg));
		}
