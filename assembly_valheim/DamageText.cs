...
public class DamageText : MonoBehaviour
{
	...
	private void UpdateWorldTexts(float dt)
	{
		...
		foreach (WorldTextInstance worldText in m_worldTexts)
		{
			...
			Color color = ((Graphic)worldText.m_textField).get_color();
			color.a = 1f - Mathf.Pow(f, 3f);
			((Graphic)worldText.m_textField).set_color(color);
	...
	private void AddInworldText(TextType type, Vector3 pos, float distance, float dmg, bool mySelf)
	{
		...
		Color color = ((type == TextType.Heal) ? new Color(0.5f, 1f, 0.5f, 0.7f) : (mySelf ? ((dmg == 0f) ? new Color(0.5f, 0.5f, 0.5f, 1f) : new Color(1f, 0f, 0f, 1f)) : (type switch
		{
		...
		{
			worldTextInstance.m_textField.set_fontSize(m_smallFontSize);
		}
		else
		{
			worldTextInstance.m_textField.set_fontSize(m_largeFontSize);
		}
		string text = type switch
		{
			TextType.TooHard => Localization.get_instance().Localize("$msg_toohard"), 
			TextType.Heal => "+" + dmg.ToString("0.#", CultureInfo.InvariantCulture), 
			TextType.Blocked => Localization.get_instance().Localize("$msg_blocked: ") + dmg.ToString("0.#", CultureInfo.InvariantCulture), 
			_ => dmg.ToString("0.#", CultureInfo.InvariantCulture), 
		};
		worldTextInstance.m_textField.set_text(text);
