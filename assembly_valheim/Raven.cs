...
public class Raven : MonoBehaviour, Hoverable, Interactable, IDestructible
{
	...
	private bool FindSpawnPoint(out Vector3 point, out GameObject landOn)
	{
		...
		for (int i = 0; i < 20; i++)
		{
			Vector3 a = Quaternion.Euler(0f, UnityEngine.Random.Range(-30, 30), 0f) * forward;
			Vector3 vector = position + a * UnityEngine.Random.Range(m_spawnDistance - 5f, m_spawnDistance);
			if (ZoneSystem.instance.GetSolidHeight(vector, out var height, out var normal, out var go) && height > ZoneSystem.instance.m_waterLevel && height > point.y && height < 2000f && normal.y > 0.5f && Mathf.Abs(height - position.y) < 2f)
			{
				vector.y = height;
				point = vector;
				landOn = go;
	...
	private bool InState(string name)
	{
		...
		if (!m_animator.get_isInitialized())
		{
			return false;
		}
