...
public class ClutterSystem : MonoBehaviour
{
	...
	public bool GetGroundInfo(Vector3 p, out Vector3 point, out Vector3 normal, out Heightmap hmap, out Heightmap.Biome biome)
	{
		...
		if (Physics.Raycast(p + Vector3.up * 500f, Vector3.down, ref val, 1000f, m_placeRayMask))
		{
			...
			hmap = ((Component)(object)((RaycastHit)(ref val)).get_collider()).GetComponent<Heightmap>();
	...
	private PatchData GenerateVegPatch(Vector2Int patchID, float size)
	{
		...
		for (int i = 0; i < m_clutter.Count; i++)
		{
			...
			for (int j = 0; j < num3; j++)
			{
				...
				if (!GetGroundInfo(vector, out var point, out var normal, out var hmap, out var biome) || (clutter.m_biome & biome) == 0)
				{
					continue;
				}
				...
				{
					float oceanDepth = hmap.GetOceanDepth(vector);
				...
				{
					vector.y = m_waterLevel;
				}
				if (clutter.m_randomOffset != 0f)
				{
					vector.y += UnityEngine.Random.Range(0f - clutter.m_randomOffset, clutter.m_randomOffset);
				}
				...
				if (clutter.m_instanced)
				{
					...
					instanceRenderer.AddInstance(vector, identity, scale);
				}
				else
				{
					GameObject item = UnityEngine.Object.Instantiate(clutter.m_prefab, vector, identity, m_grassRoot.transform);
